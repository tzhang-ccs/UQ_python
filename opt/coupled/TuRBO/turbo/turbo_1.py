...
class Turbo1:
    ...
    def get_paras(self, p_names, bgn_id, end_id):
        p_dataset = pd.DataFrame()

        for iid in range(bgn_id,end_id+1):
            case_path = f'/home/ac.tzhang/fs0_large/E3SM_archive/workdir.{iid}/atm_in'
            tmp = {}
            for pn in p_names:
            ...
            tmp = pd.DataFrame(tmp,index=[iid])
            p_dataset = pd.concat([p_dataset,tmp])

        return p_dataset


    def get_score(self, path_output_case, outputs, output_cons):
    ...
    def optimize(self,p_names, outputs, output_cons,  sampling_flag=True):
        ...
        while self.n_evals < self.max_evals:
            ...
            if sampling_flag == True:
                ...
                bgn_id = 9
                end_id = 87
                X_init = self.get_paras(p_names, bgn_id, end_id)
                X_init = X_init.values

                nsample = end_id - bgn_id + 1
                self.n_init = nsample

                fX_init = np.zeros((nsample,1))
                for iid in range(bgn_id, end_id+1):
                    fX_init[iid-bgn_id] = self.get_score(f'workdir.{iid}', outputs, output_cons)
            ...
            #print(self.X.shape, X_init.shape)
            #print(self.fX.shape, fX_init.shape)
